...
df_restaurant_monde = pd.read_csv('data/df_restaurant_monde.csv', sep=',')

st.title("Hello Worlddddd :earth_africa:")
type = st.multiselect(
    "Qu'est-ce qu'il vous ferait plaisir ? :fork_and_knife:",
    df_restaurant_monde['type_food_2'].unique())
livraison = st.sidebar.radio("Options de livraison :car:", df_restaurant_monde['extracted_options'].unique())

with_promo = st.sidebar.checkbox("Avec offres promotionnelles", key="promo_checkbox_word")

st.sidebar.write(" ")

min_rating, max_rating = st.sidebar.slider('**Notes :**', min_value=0.0, max_value=5.0, value=(0.0, 5.0), step=0.1, key="rating_slider_world")
...
if not type:
    type = df_restaurant_monde['type_food_2'].unique()
    
filtered_df = df_restaurant_monde[
    (df_restaurant_monde['type_food_2'].isin(type)) &
    (df_restaurant_monde['extracted_options'] == livraison)
]
