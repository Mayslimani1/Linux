...
df_boisson = pd.read_csv('data/df_boisson.csv', sep=',')

st.title("Envie de boire un verre ? :cocktail:")
type = st.multiselect(
    "Que voulez-vous boire ? :smile:",
    df_boisson['type_food_2'].unique())
livraison = st.sidebar.radio("Options de livraison :car:", df_boisson['extracted_options'].unique())

with_promo = st.sidebar.checkbox("Avec offres promotionnelles", key="promo_checkbox_boisson")

st.sidebar.write(" ")

min_rating, max_rating = st.sidebar.slider('**Notes :**', min_value=0.0, max_value=5.0, value=(0.0, 5.0), step=0.1, key="rating_slider_boisson")
...
if not type:
    type = df_boisson['type_food_2'].unique()
    
filtered_df = df_boisson[
    (df_boisson['type_food_2'].isin(type)) &
    (df_boisson['extracted_options'] == livraison)
]
